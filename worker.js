...
addEventListener('fetch',function(event){
  event.respondWith((async function(r){
    ...
    // ===== AI SOLVE with Working Out =====
    if(path==='/api/ai/solve'&&r.method==='POST'){b=await r.json();if(!b.question)return json({error:'question required'},400);
      // Use typeof check to support both Service Worker (global) and ES Module (env) formats
      var GEMINI_KEY=typeof GEMINI_KEY!=='undefined'?GEMINI_KEY:(typeof env!=='undefined'?env.GEMINI_KEY:null);
      var GROQ_KEY=typeof GROQ_KEY!=='undefined'?GROQ_KEY:(typeof env!=='undefined'?env.GROQ_KEY:null);
      var MISTRAL_KEY=typeof MISTRAL_KEY!=='undefined'?MISTRAL_KEY:(typeof env!=='undefined'?env.MISTRAL_KEY:null);
      var providers=[];
      if(GEMINI_KEY)providers.push('gemini');
      if(GROQ_KEY)providers.push('groq');
      if(MISTRAL_KEY)providers.push('mistral');
      if(!providers.length)return json({error:'No AI key configured. Set GEMINI_KEY, GROQ_KEY, or MISTRAL_KEY in Worker env vars'},400);
      ...
      var answer,working,resp,d,text,aMatch;
      if(provider==='gemini'){
        resp=await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key='+GEMINI_KEY,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:0.7,maxOutputTokens:1024}})});
        d=await resp.json();text=(d&&d.candidates&&d.candidates[0]&&d.candidates[0].content&&d.candidates[0].content.parts&&d.candidates[0].content.parts[0]&&d.candidates[0].content.parts[0].text||'').trim();
        aMatch=text.match(/ANSWER:\s*(.+)/i);answer=aMatch?aMatch[1].trim():text.split('\n').pop().trim();working=text.replace(/ANSWER:\s*.+/i,'').trim()}
      else if(provider==='groq'){
        resp=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+GROQ_KEY},body:JSON.stringify({model:'mixtral-8x7b-32768',messages:[{role:'user',content:prompt}],temperature:0.7,max_tokens:1024})});
        d=await resp.json();text=(d&&d.choices&&d.choices[0]&&d.choices[0].message&&d.choices[0].message.content||'').trim();
        aMatch=text.match(/ANSWER:\s*(.+)/i);answer=aMatch?aMatch[1].trim():text.split('\n').pop().trim();working=text.replace(/ANSWER:\s*.+/i,'').trim()}
      else if(provider==='mistral'){
        resp=await fetch('https://api.mistral.ai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+MISTRAL_KEY},body:JSON.stringify({model:'mistral-large-latest',messages:[{role:'user',content:prompt}],temperature:0.7,max_tokens:1024})});
        d=await resp.json();text=(d&&d.choices&&d.choices[0]&&d.choices[0].message&&d.choices[0].message.content||'').trim();
        aMatch=text.match(/ANSWER:\s*(.+)/i);answer=aMatch?aMatch[1].trim():text.split('\n').pop().trim();working=text.replace(/ANSWER:\s*.+/i,'').trim()}
    ...
    if(path==='/api/lnut/login'&&r.method==='POST'){b=await r.json();if(!b.username||!b.password)return json({error:'username and password required'},400);
      ...
      resp=await fetch('https://api.languagenut.com/loginController/attemptLogin?'+new URLSearchParams({username:b.username,pass:b.password,friendlyCaptchaToken:fToken}),{
